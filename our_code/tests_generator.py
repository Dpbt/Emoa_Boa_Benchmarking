...
def simple_map_tests_generator(num_tests: int = 50,
    ...
    for exp_number in range(1, num_tests + 1):
        ...
        _, _ = grid_generator(width=width,
                              ...
                              map_name=f"../generated_maps/{num_dims}_dims_{walls_percent}_ratio/{map_name}_{num_dims}_{width}_{height}_{walls_percent}_{exp_number}")

        map_names_list = []

        for dim in range(1, num_dims + 1):
            map_names_list.append(f"../generated_maps/{num_dims}_dims_{walls_percent}_ratio/{map_name}_{num_dims}_{width}_{height}_{walls_percent}_{exp_number}_{dim}.gr")

        emoa_test = [exp_number, "emoa", f"simple map {num_dims} dims", time_limit, start, finish,
                     f"../data_out/simple_map_{num_dims}.txt", map_names_list]
